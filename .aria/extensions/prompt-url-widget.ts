...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@aaditri-globaltech/aria-coding-agent";
import { Container, Text } from "@aaditri-globaltech/aria-tui";
...
async function fetchAdvisoryMetadata(aria: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await aria.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(aria, cwd, target);
	}
	...
	try {
		const result = await aria.exec("gh", args);
...
export default function promptUrlWidgetExtension(aria: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = aria.getSessionName()?.trim();
		if (!currentName) {
			aria.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(aria, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	aria.on("session_switch", async (_event, ctx) => {
