...
Aria and also where the boundaries are.

In general Aria is a coding agent that runs locally within the security boundary
...
Aria treats the local user account and files writable by that account as inside
the same trust boundary as the Aria process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Aria configuration, they can generally influence Aria or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Aria grants that write access or crosses
an operating-system privilege boundary.

Aria relies on users installing trustworthy extensions and loading trustworthy
skills and only to use Aria within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in Aria or another package in
this repository, please report it privately by either:

- Emailing `security@aaditriglobaltech.com`, or
...
repository code are in scope as well as Aaditri GlobalTech-operated infrastructure
on `aria.dev`.

## Out Of Scope

- Local code execution or sandboxing behavior (the Aria coding agent intentionally does not have a sandbox)
- Behavior of Aria extensions or skills installed by the user
...
- Public internet exposure of an Aria installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.aria`,
  `~/.aria/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Aria itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the Aria coding agent.
...
For example, a report showing that malicious contents written to a trusted Aria
configuration file cause Aria to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Aria.  For exposed-secret reports,
include evidence that the credential is owned by Aaditri GlobalTech or grants access to
Aaditri GlobalTech-operated infrastructure or services.
