...
import type { GitChange, GitStatus } from "../../../shared/workspace";
...
export function SourceControlSidebar(props: SourceControlSidebarProps) {
  ...
  const loadStatus = async (cwd: string) => {
    ...
    setError(undefined);
    try {
      const next = await window.electron.workspace.gitStatus(cwd);
  ...
  const stage = (path: string) => {
    const cwd = props.cwd;
    if (!cwd) return;
    void runAction(() => window.electron.workspace.gitStage(cwd, path));
  };

  const unstage = (path: string) => {
    const cwd = props.cwd;
    if (!cwd) return;
    void runAction(() => window.electron.workspace.gitUnstage(cwd, path));
  };
  ...
  const commit = () => {
    ...
    void runAction(async () => {
      await window.electron.workspace.gitCommit(cwd, message());
      setMessage("");
    });
  };
