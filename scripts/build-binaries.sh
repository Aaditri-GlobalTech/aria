#!/usr/bin/env bash
#
# Build aria binaries for all platforms locally.
...
#     aria-darwin-arm64.tar.gz
#     aria-darwin-x64.tar.gz
#     aria-linux-x64.tar.gz
#     aria-linux-arm64.tar.gz
#     aria-windows-x64.zip
#     aria-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/aria.exe"
    else
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/aria"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating aria-$platform.zip..."
        (cd "$platform" && zip -r ../aria-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating aria-$platform.tar.gz..."
        mv "$platform" aria && tar -czf aria-$platform.tar.gz aria && mv aria "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../aria-$platform.zip)
    else
        tar -xzf aria-$platform.tar.gz && mv aria "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/aria.exe"
    else
        echo "  $OUTPUT_DIR/$platform/aria"
    fi
done
