...
const agentDirEnvName = "ARIA_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "ARIA_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific ARIA_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force ARIA_OFFLINE=1 / ARIA_SKIP_VERSION_CHECK=1
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "aria-startup-benchmark-")) : undefined;
