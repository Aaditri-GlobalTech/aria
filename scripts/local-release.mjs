...
const packages = [
	{ directory: "packages/ai", name: "@aaditri-globaltech/aria-ai" },
	{ directory: "packages/tui", name: "@aaditri-globaltech/aria-tui" },
	{ directory: "packages/agent", name: "@aaditri-globaltech/aria-agent" },
	{ directory: "packages/coding-agent", name: "@aaditri-globaltech/aria-coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "aria-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `aria-${platform}.zip` : `aria-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createAriaShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "aria.cmd"))) {
			writeFileSync(join(installDirectory, "aria.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\aria.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "aria.ps1"), '& "$PSScriptRoot/node_modules/.bin/aria.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "aria.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\aria.exe" %*\r\n');
		writeFileSync(join(installDirectory, "aria.ps1"), '& "$PSScriptRoot/node_modules/.bin/aria.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "aria"), join(installDirectory, "aria"));
}
...
if (rootPackageJson.name !== "aria-monorepo") {
...
if (!options.skipInstall) {
	...
	createAriaShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createAriaShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `aria-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "aria.exe" : "aria")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "aria.cmd" : "aria")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "aria.cmd" : "aria")} --help`);
	}
}
