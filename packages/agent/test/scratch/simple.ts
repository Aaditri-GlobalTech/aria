...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".aria/skills")),
		source("user", join(homedir(), ".aria/agent/skills")),
		source("path", join(env.cwd, "../../../aria-skills")),
	],
	(skill, source) => ({ ...skill, source }),
);
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".aria/prompts")), source("user", join(homedir(), ".aria/agent/prompts"))],
