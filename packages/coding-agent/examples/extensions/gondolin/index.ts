 ...
 *
 * Runs aria's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   aria -e /path/to/aria/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (aria: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `aria ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	aria.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
