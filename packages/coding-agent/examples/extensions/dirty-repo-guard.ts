...
async function checkDirtyRepo(
	...
	const { stdout, code } = await aria.exec("git", ["status", "--porcelain"]);
...
export default function (aria: ExtensionAPI) {
	aria.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(aria, ctx, action);
	});

	aria.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(aria, ctx, "fork");
	});
}
