 ...
 *
 *   mkdir -p ~/.aria/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.aria/agent/extensions/
 *
 * Or:
 *
 *   aria -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .aria, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@aaditri-globaltech/aria-coding-agent";
...
export default function (aria: ExtensionAPI) {
	...
	aria.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
