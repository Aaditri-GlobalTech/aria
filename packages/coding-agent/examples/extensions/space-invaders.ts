...
import { isKeyRelease, Key, matchesKey, visibleWidth } from "@aaditri-globaltech/aria-tui";
		...
		handler: async (_args, ctx) => {
			...
			await ctx.ui.custom((tui, _theme, _kb, done) => {
				return new SpaceInvadersComponent(
					...
					() => done(undefined),
					(state) => {
						aria.appendEntry(INVADERS_SAVE_TYPE, state);
					},
					savedState,
				);
			});
