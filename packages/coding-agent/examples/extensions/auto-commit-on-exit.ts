...
export default function (aria: ExtensionAPI) {
	aria.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await aria.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[aria] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await aria.exec("git", ["add", "-A"]);
		const { code: commitCode } = await aria.exec("git", ["commit", "-m", commitMessage]);
