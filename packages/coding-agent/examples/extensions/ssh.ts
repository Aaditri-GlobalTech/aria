 ...
 *
 * Usage:
 *   aria -e ./ssh.ts --ssh user@host
 *   aria -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (aria: ExtensionAPI) {
	aria.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	aria.on("session_start", async (_event, ctx) => {
		...
		const arg = aria.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
