 ...
 * (logo + keybinding hints) with a custom component showing the aria mascot.
 */

import type { ExtensionAPI, Theme } from "@aaditri-globaltech/aria-coding-agent";
import { VERSION } from "@aaditri-globaltech/aria-coding-agent";

// --- ARIA MASCOT ---
// Based on the aria mascot - the aria agent character
function getAriaMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const ariaBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${ariaBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${ariaBlue(BLOCK.repeat(2))}    ${ariaBlue(BLOCK.repeat(2))}`;
...
export default function (aria: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getAriaMascot(theme);
	...
	aria.registerCommand("builtin-header", {
