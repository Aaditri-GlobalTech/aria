...
async function resolveGitHubRepo(aria: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await aria.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (aria: ExtensionAPI): void {
	aria.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(aria, ctx.cwd);
		if (!resolvedRepo.ok) {
