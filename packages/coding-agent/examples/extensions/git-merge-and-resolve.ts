 ...
 *
 * Start aria with this extension:
 *   aria -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(aria: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await aria.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (aria: ExtensionAPI) {
	aria.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await aria.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await aria.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await aria.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await aria.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await aria.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await aria.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(aria, ctx.cwd);
		if (conflicts.length === 0) return;

		aria.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}
