...
import type { Component, EditorTheme, TUI } from "@aaditri-globaltech/aria-tui";
import { truncateToWidth, visibleWidth } from "@aaditri-globaltech/aria-tui";
...
export default function (aria: ExtensionAPI) {
	...
	aria.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await aria.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = aria.getThinkingLevel();
