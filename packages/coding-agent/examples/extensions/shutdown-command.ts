 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit aria cleanly.
 */
...
export default function (aria: ExtensionAPI) {
	// Register a /quit command that cleanly exits aria
	aria.registerCommand("quit", {
		description: "Exit aria cleanly",
	...
	aria.registerTool({
		...
		description: "Complete a task and exit aria",
	...
	aria.registerTool({
		...
		description: "Deploy the application and exit aria",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await aria.exec("npm", ["run", "deploy", params.environment], { signal });
