...
Example extensions for aria-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
aria --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.aria/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `aria.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs aria theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `aria.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using aria-ai's built-in Anthropic/OpenAI streaming via proxy |
