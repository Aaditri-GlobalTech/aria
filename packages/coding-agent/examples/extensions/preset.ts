 ...
 * - ~/.aria/agent/presets.json (global)
 * - <cwd>/.aria/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `aria --preset plan` - start with plan preset
...
import type { Api, Model } from "@aaditri-globaltech/aria-ai";
import type { ExtensionAPI, ExtensionContext } from "@aaditri-globaltech/aria-coding-agent";
import { DynamicBorder, getAgentDir } from "@aaditri-globaltech/aria-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@aaditri-globaltech/aria-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".aria", "presets.json");
...
export default function presetExtension(aria: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: aria.getThinkingLevel(),
				tools: aria.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			aria.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = aria.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				aria.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.aria/agent/presets.json or .aria/presets.json",
				"warning",
			);
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await aria.setModel(originalState.model);
				}
				aria.setThinkingLevel(originalState.thinkingLevel);
				aria.setActiveTools(originalState.tools);
			} else {
				aria.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	aria.registerShortcut(Key.ctrlShift("u"), {
	...
	aria.on("before_agent_start", async (event) => {
	...
	aria.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = aria.getFlag("preset");
	...
	aria.on("turn_start", async () => {
		if (activePresetName) {
			aria.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
