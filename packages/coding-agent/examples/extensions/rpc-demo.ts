...
export default function (aria: ExtensionAPI) {
	...
	aria.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "aria RPC Demo (new session)" : "aria RPC Demo");
	...
	aria.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	aria.on("turn_end", async (_event, ctx) => {
	...
	aria.registerCommand("rpc-input", {
	...
	aria.registerCommand("rpc-editor", {
	...
	aria.registerCommand("rpc-prefill", {
