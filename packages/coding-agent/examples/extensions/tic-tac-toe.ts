...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@aaditri-globaltech/aria-coding-agent";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@aaditri-globaltech/aria-tui";
...
export default function (aria: ExtensionAPI) {
	aria.on("session_start", async (_event, ctx) => reconstructState(ctx));
	aria.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	aria.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	aria.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	aria.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			aria.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						aria.appendEntry(SAVE_TYPE, getBoardDetails());
