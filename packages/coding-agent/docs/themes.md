> aria can create themes. Ask it to build one for your setup.
...
Aria loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.aria/agent/themes/*.json`
- Project: `.aria/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `aria.themes` entries in `package.json`
...
On first run, aria detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.aria/agent/themes
vim ~/.aria/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Aaditri-GlobalTech/aria/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, aria reloads it automatically for immediate visual feedback.
...
Aria uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, aria falls back to the nearest approximation.
