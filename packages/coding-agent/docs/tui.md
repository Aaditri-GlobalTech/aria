> aria can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@aaditri-globaltech/aria-tui`](https://github.com/Aaditri-GlobalTech/aria/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@aaditri-globaltech/aria-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `ARIA_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@aaditri-globaltech/aria-tui";
...
**In custom tools** via `aria.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = aria.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@aaditri-globaltech/aria-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@aaditri-globaltech/aria-tui";
```
...
```typescript
import { matchesKey, Key } from "@aaditri-globaltech/aria-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@aaditri-globaltech/aria-tui";
...
} from "@aaditri-globaltech/aria-tui";
...
```typescript
import { getMarkdownTheme } from "@aaditri-globaltech/aria-coding-agent";
import { Markdown } from "@aaditri-globaltech/aria-tui";
...
Set `ARIA_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
ARIA_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@aaditri-globaltech/aria-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@aaditri-globaltech/aria-coding-agent";
import { DynamicBorder } from "@aaditri-globaltech/aria-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@aaditri-globaltech/aria-tui";
...
```typescript
import { BorderedLoader } from "@aaditri-globaltech/aria-coding-agent";

aria.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@aaditri-globaltech/aria-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@aaditri-globaltech/aria-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@aaditri-globaltech/aria-tui";

aria.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while aria is streaming a response.
...
// Restore aria's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@aaditri-globaltech/aria-tui";
