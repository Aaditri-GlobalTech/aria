...
Extensions can register custom model providers via `aria.registerProvider()`. This enables:
...
export default function (aria: ExtensionAPI) {
  ...
  aria.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. aria waits for the factory before startup continues, so the provider is available during interactive startup and to `aria --list-models`.
...
Use `aria.unregisterProvider(name)` to remove a provider that was previously registered via `aria.registerProvider(name, ...)`:
...
// Later, remove it
aria.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map aria levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
aria.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@aaditri-globaltech/aria-ai";
...
Credentials are persisted in `~/.aria/agent/auth.json`:
...
- [anthropic.ts](https://github.com/Aaditri-GlobalTech/aria/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/Aaditri-GlobalTech/aria/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/Aaditri-GlobalTech/aria/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/Aaditri-GlobalTech/aria/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/Aaditri-GlobalTech/aria/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/Aaditri-GlobalTech/aria/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, aria can recover automatically by compacting the conversation and retrying. This recovery only kicks in if aria recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of aria's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/Aaditri-GlobalTech/aria/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message aria does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase aria recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before aria tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what aria checks. With this in place, aria will:
...
- Match a provider-specific pattern, not aria's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of aria's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/Aaditri-GlobalTech/aria/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps aria thinking levels to provider/model-specific values; null marks a level unsupported. */
