> aria can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to aria's agent capabilities. Use it to embed aria in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @aaditri-globaltech/aria-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@aaditri-globaltech/aria-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `aria.sendMessage()`.
...
The `Agent` class (from `@aaditri-globaltech/aria-agent`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.aria/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.aria/extensions/`)
- Project skills:
  - `.aria/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.aria/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.aria/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@aaditri-globaltech/aria-coding-agent";
...
// Default: uses ~/.aria/agent/auth.json and ~/.aria/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@aaditri-globaltech/aria-coding-agent";
...
The `edit` tool returns `details.diff` for Aria's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@aaditri-globaltech/aria-coding-agent";
...
import { createAgentSession, defineTool } from "@aaditri-globaltech/aria-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `aria.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `aria.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.aria/agent/extensions/`, `.aria/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `aria.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@aaditri-globaltech/aria-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@aaditri-globaltech/aria-coding-agent";
...
1. Global: `~/.aria/agent/settings.json`
2. Project: `<cwd>/.aria/settings.json`
