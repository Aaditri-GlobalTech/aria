...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `aria-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(aria) => {
				aria.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(aria) => {
				aria.on("session_start", () => {
					aria.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(aria) => {
				aria.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						aria.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
