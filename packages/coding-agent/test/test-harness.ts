...
import { createAssistantMessageEventStream } from "@aaditri-globaltech/aria-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `aria-harness-${Date.now()}-${Math.random().toString(36).slice(2)}`);
