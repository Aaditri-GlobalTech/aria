...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "aria-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("ARIA_CODING_AGENT_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.ARIA_CODING_AGENT_DIR!, "themes", "foo.json");
