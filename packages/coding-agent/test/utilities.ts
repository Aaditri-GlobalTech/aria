...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@aaditri-globaltech/aria-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.aria/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".aria", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.aria/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.aria/agent/auth.json
 */
...
/** Path to the real aria agent config directory */
export const ARIA_AGENT_DIR = join(homedir(), ".aria", "agent");

/**
 * Get an AuthStorage instance backed by ~/.aria/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `aria-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
