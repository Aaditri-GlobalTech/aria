...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (aria: ExtensionAPI) => {
			aria.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (aria: ExtensionAPI) => {
			aria.on("session_compact", async (event: SessionCompactEvent) => {
