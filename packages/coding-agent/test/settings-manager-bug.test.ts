 ...
 * 1. Aria starts with settings.json containing packages: ["npm:some-pkg"]
...
describe("SettingsManager - External Edit Preservation", () => {
	...
	it("should preserve file changes to packages array when changing unrelated setting", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				theme: "dark",
				packages: ["npm:aria-mcp-adapter"],
			}),
		);

		// Aria starts up, loads settings into memory
		const manager = SettingsManager.create(projectDir, agentDir);

		// At this point, globalSettings.packages = ["npm:aria-mcp-adapter"]
		expect(manager.getPackages()).toEqual(["npm:aria-mcp-adapter"]);
