...
/**
 * Regression test for https://github.com/Aaditri-GlobalTech/aria/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "aria-2791-"));
...
process.env.ARIA_CODING_AGENT_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync(process.execPath, [scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, ARIA_CODING_AGENT_DIR: agentDir },
