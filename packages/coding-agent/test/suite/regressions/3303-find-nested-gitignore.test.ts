...
/**
 * Regression test for https://github.com/Aaditri-GlobalTech/aria/issues/3303
 *
...
describe("issue #3303 nested .gitignore rules leak into sibling directories", () => {
	...
	describe("flat sibling case", () => {
		beforeEach(() => {
			tempRoot = mkdtempSync(join(tmpdir(), "aria-3303-flat-"));
	...
	describe("deeply nested case", () => {
		beforeEach(() => {
			tempRoot = mkdtempSync(join(tmpdir(), "aria-3303-deep-"));
