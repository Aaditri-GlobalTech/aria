...
// Regression for https://github.com/Aaditri-GlobalTech/aria/issues/5080
//
...
function createSessionManager(options: { sessionFile?: string } = {}): SessionManager {
	return {
		...
		getSessionDir: () => "/tmp/aria-sessions",
		usesDefaultSessionDir: () => true,
	} as unknown as SessionManager;
}

function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "aria-shutdown-resume-hint-"));
	tempDirs.push(dir);
