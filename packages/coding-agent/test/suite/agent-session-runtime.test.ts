...
import { fauxAssistantMessage, registerFauxProvider } from "@aaditri-globaltech/aria-ai";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const tempDir =
			options?.cwd ?? join(tmpdir(), `aria-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const runtimeOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(aria: ExtensionAPI) => {
						aria.registerProvider(faux.getModel().provider, {
	...
	it("persists message_end assistant replacements to the session manager", async () => {
		const { runtime } = await createRuntimeForTest((aria: ExtensionAPI) => {
			aria.on("message_end", (event) => {
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtime } = await createRuntimeForTest((aria: ExtensionAPI) => {
			aria.on("session_before_switch", (event) => {
				events.push(event);
			});
			aria.on("session_shutdown", (event) => {
				events.push(event);
			});
			aria.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("honors session_before_switch cancellation for new and resume", async () => {
		...
		events.length = 0;
		const otherDir = join(tmpdir(), `aria-runtime-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtime } = await createRuntimeForTest((aria: ExtensionAPI) => {
			aria.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
	...
	it("duplicates the current active branch in-memory when forking at the current position", async () => {
		const tempDir = join(
			tmpdir(),
			`aria-runtime-suite-in-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		mkdirSync(tempDir, { recursive: true });
	...
	it("updates the runtime session cwd on cross-cwd session replacement", async () => {
		const firstDir = join(tmpdir(), `aria-runtime-cwd-a-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const secondDir = join(tmpdir(), `aria-runtime-cwd-b-${Date.now()}-${Math.random().toString(36).slice(2)}`);
