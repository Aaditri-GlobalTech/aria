...
import { fauxAssistantMessage, fauxToolCall } from "@aaditri-globaltech/aria-ai";
			...
			extensionFactories: [
				(aria: ExtensionAPI) => {
					aria.on("agent_end", async () => {
						if (sent) return;
						sent = true;
						aria.sendUserMessage("conflict report", { deliverAs: "followUp" });
					});
				},
			],
		});
