...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const extensionFile = join(cachedDir, "aria-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "aria-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ aria: { extensions: ["./aria-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "aria-extensions/session-breakdown.ts")).toBe("// fresh");
		});
		...
		it("should not refresh pinned temporary git sources", async () => {
			...
			expect(getFileContent(cachedDir, "aria-extensions/session-breakdown.ts")).toBe("// pinned");
		});
	});
	...
	describe("scope-aware update", () => {
		it("should not install locally when source is only registered globally", async () => {
			...
			const projectGitDir = join(tempDir, ".aria", "git", "github.com", "test", "extension");
