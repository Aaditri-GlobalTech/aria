...
describe("ExtensionRunner", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aria-runner-test-"));
	...
	aria.on("project_trust", () => ({ trusted: "undecided", remember: true }));
}`,
			);
			fs.writeFileSync(
				decidedPath,
				`export default function(aria) {
	aria.on("project_trust", () => ({ trusted: "no", remember: true }));
}`,
			);
	...
	describe("shortcut conflicts", () => {
		...
		it("warns but allows when extension uses non-reserved built-in shortcut", async () => {
			...
			const extCode = `
				export default function(aria) {
					aria.registerShortcut("${pasteImageKey}", {
		...
		it("warns when two extensions register same shortcut", async () => {
			...
			const extCode1 = `
				export default function(aria) {
					aria.registerShortcut("ctrl+shift+x", {
	...
	describe("command collection", () => {
		...
		it("suffixes duplicate extension commands in insertion order", async () => {
			const cmdCode = (description: string) => `
				export default function(aria) {
					aria.registerCommand("shared-cmd", {
	...
	describe("message renderers", () => {
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(aria) {
					aria.registerMessageRenderer("my-type", (message, options, theme) => null);
				}
			`;
	...
	describe("before_agent_start", () => {
		it("keeps ctx.getSystemPrompt() in sync with chained system prompt updates", async () => {
			const extCode1 = `
				export default function(aria) {
					aria.on("before_agent_start", async (_event, ctx) => {
						return {
	...
	describe("tool_result chaining", () => {
		it("chains content modifications across handlers", async () => {
			const extCode1 = `
				export default function(aria) {
					aria.on("tool_result", async (event) => {
						return {
	...
	describe("hasHandlers", () => {
		it("returns true when handlers exist for event type", async () => {
			const extCode = `
				export default function(aria) {
					aria.on("tool_call", async () => undefined);
				}
			`;
