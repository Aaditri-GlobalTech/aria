...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates aria starting up)
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".aria", "settings.json");
	...
	describe("project trust", () => {
		it("should skip project settings when project is not trusted", () => {
			...
			writeFileSync(join(projectDir, ".aria", "settings.json"), JSON.stringify({ theme: "project" }));
		...
		it("should read default project trust from global settings only", () => {
			...
			writeFileSync(join(projectDir, ".aria", "settings.json"), JSON.stringify({ defaultProjectTrust: "never" }));
	...
	describe("project settings directory creation", () => {
		it("should not create .aria folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .aria folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .aria folder that beforeEach created
			rmSync(join(projectDir, ".aria"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .aria folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".aria"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .aria folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .aria folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .aria folder should NOT exist yet
			expect(existsSync(join(projectDir, ".aria"))).toBe(false);
			...
			// Now .aria folder should exist
			expect(existsSync(join(projectDir, ".aria"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".aria", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".aria", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".aria", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
