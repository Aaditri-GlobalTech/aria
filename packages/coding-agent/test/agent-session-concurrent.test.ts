...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `aria-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(aria) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = aria;
			},
			(aria) => {
		...
		aria!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
