...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.ARIA_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.ARIA_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .aria skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .aria", async () => {
			const extDir = join(tempDir, ".aria", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".aria"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".aria", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".aria", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".aria", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".aria", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".aria", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json aria.extensions in extensions setting", async () => {
			// Create a package with aria.extensions in package.json
			...
			// Should find the extensions declared in package.json aria.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .aria/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-aria", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-aria"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-aria\ndescription: user aria\n---\n");
		...
		it("should use the project .aria dir as baseDir for project .aria skills", async () => {
			const projectBaseDir = join(tempDir, ".aria");
			const skillPath = join(projectBaseDir, "skills", "project-aria", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-aria"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-aria\ndescription: project aria\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".aria", "agent");
		...
		it("should dedupe user skill entries when ~/.aria/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .aria auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".aria\n");

			const skillDir = join(tempDir, ".aria", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with aria manifest", async () => {
		...
		it("should keep aria manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.aria\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".aria", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the aria-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "aria-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .aria settings base", () => {
			...
			const rel = relative(join(tempDir, ".aria"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in aria manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json aria.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".aria", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".aria", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".aria", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".aria", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".aria", "npm", "node_modules", "example");
