...
function isOfflineModeEnabled(): boolean {
	const value = process.env.ARIA_OFFLINE;
...
type SkillDiscoveryMode = "aria" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "aria" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readAriaManifestFile(packageJsonPath: string): AriaManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { aria?: AriaManifest };
		return pkg.aria ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readAriaManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "aria");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside Aria and resolve Aria APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @aaditri-globaltech/aria-* peers. Stale auto-installed Aria peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "aria-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = this.readAriaManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof AriaManifest];
	...
	private readAriaManifest(packageRoot: string): AriaManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			...
			// Project skills from .aria/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "aria"),
				projectMetadata,
		...
		// User extensions from ~/.aria/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.aria/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "aria"),
			userMetadata,
