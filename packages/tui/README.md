...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "@aaditri-globaltech/aria-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the `TUI` constructor option, `setShowHardwareCursor(true)`, or `ARIA_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { CombinedAutocompleteProvider } from "@aaditri-globaltech/aria-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@aaditri-globaltech/aria-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@aaditri-globaltech/aria-tui";
import type { Component } from "@aaditri-globaltech/aria-tui";
...
```bash
ARIA_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
