...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.ARIA_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.ARIA_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.ARIA_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".aria", "agent", "aria-debug.log");
		...
		// Configurable via setClearOnShrink() or ARIA_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".aria", "agent", "aria-crash.log");
				const crashData = [
		...
		if (process.env.ARIA_TUI_DEBUG === "1") {
