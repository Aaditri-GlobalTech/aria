...
describe.skipIf(!oauthToken)("Anthropic OAuth tool name normalization", () => {
	...
	it("should handle aria's built-in tools (read, write, edit, bash)", async () => {
		// Aria's tools use lowercase names, CC uses PascalCase
		const readTool: Tool = {
			name: "read",
	...
	it("should NOT map find to Glob - find is not a CC tool name", async () => {
		// Aria has a "find" tool, CC has "Glob" - these are DIFFERENT tools
