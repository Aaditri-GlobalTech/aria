...
const originalAgentDir = process.env.ARIA_CODING_AGENT_DIR;
...
afterEach(() => {
	...
	if (originalAgentDir === undefined) {
		...
		process.env.ARIA_CODING_AGENT_DIR = originalAgentDir;
	}
	resetOpenAICodexWebSocketDebugStats();
	vi.useRealTimers();
	vi.restoreAllMocks();
...
describe("openai-codex streaming", () => {
	it("streams SSE responses into AssistantMessageEventStream", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "aria-codex-stream-"));
		process.env.ARIA_CODING_AGENT_DIR = tempDir;
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				...
				expect(headers?.get("originator")).toBe("aria");
